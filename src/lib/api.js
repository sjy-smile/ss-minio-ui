...
const taskInfo = (identifier) => {
    return http.get(`/process/${identifier}`)
}
...
const initTask = ({ identifier, fileName, totalSize, chunkSize }) => {
    return http.post('/initTask', {identifier, fileName, totalSize, chunkSize})
}
...
const preSignUrl = ({ identifier, partNumber }) => {
    return http.get(`/sign/${identifier}/${partNumber}`)
}
...
const merge = (identifier) => {
    return http.post(`/merge/${identifier}`)
}
